...
const languages = [
    ...
    { code: 'mr', name: 'Marathi', flag: 'https://flagcdn.com/in.svg' },
    { code: 'gu', name: 'Gujarati', flag: 'https://flagcdn.com/in.svg' },
    { code: 'kn', name: 'Kannada', flag: 'https://flagcdn.com/in.svg' },
    { code: 'ml', name: 'Malayalam', flag: 'https://flagcdn.com/in.svg' },
    { code: 'pa', name: 'Punjabi', flag: 'https://flagcdn.com/in.svg' },
    { code: 'ur', name: 'Urdu', flag: 'https://flagcdn.com/in.svg' },
    { code: 'or', name: 'Odia', flag: 'https://flagcdn.com/in.svg' },
    { code: 'as', name: 'Assamese', flag: 'https://flagcdn.com/in.svg' },
    { code: 'ks', name: 'Kashmiri', flag: 'https://flagcdn.com/in.svg' },
    { code: 'sd', name: 'Sindhi', flag: 'https://flagcdn.com/in.svg' }
  ];
      ...
      <div className="p-8 w-full max-w-4xl bg-white text-black shadow-2xl rounded-2xl overflow-hidden">
        ...
        <div className="flex items-center gap-4 mb-6">
          <div className="flex items-center gap-2">
            <img src={languages.find(l => l.code === sourceLanguage)?.flag} alt="Flag" className="w-6 h-6" />
            <select className="border p-2 rounded-lg" value={sourceLanguage} onChange={(e) => setSourceLanguage(e.target.value)}>
              {languages.map(lang => <option key={lang.code} value={lang.code}>{lang.name}</option>)}
            </select>
          </div>

          <button className="text-2xl p-2" onClick={swapLanguages}>↔️</button>

          <div className="flex items-center gap-2">
            <img src={languages.find(l => l.code === targetLanguage)?.flag} alt="Flag" className="w-6 h-6" />
            <select className="border p-2 rounded-lg" value={targetLanguage} onChange={(e) => setTargetLanguage(e.target.value)}>
              {languages.map(lang => <option key={lang.code} value={lang.code}>{lang.name}</option>)}
            </select>
          </div>
        </div>

        <textarea className="w-full p-3 border rounded-lg mb-4 h-40" placeholder="Write your text here..." value={inputText} onChange={(e) => setInputText(e.target.value)}></textarea>
        
        <button className="w-full bg-blue-600 text-white p-3 rounded-lg hover:bg-blue-700" onClick={translateText} disabled={loading}>
